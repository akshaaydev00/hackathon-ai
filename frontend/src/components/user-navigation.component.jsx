import { Link } from "react-router-dom"
import AnimatioWrapper from "../common/page-animation"
import { useContext } from "react"
import { UserContext } from "../App"
import { removeFromSession } from "../common/session"

const UserNavigationPanel = () => {
    const {userAuth, setUserAuth} = useContext(UserContext)
    const studentid = userAuth.studentid

    const signOutUser = () => {
        removeFromSession("user")
        setUserAuth({access_token: null})
    }
    return (
         <AnimatioWrapper transition={{ duration:0.2}} classname="absolute right-0 z-50 ">
            
           <div className="bg-white absolute right-0 border border-grey w-60 overflow-hidden duration-200">


            <Link to="/editor" className="flex gap-2 link md:hidden pl-8 py-4 ">
                
            </Link>
            
            <Link to={`/user/${studentid}`} className="link pl-8 py-4">Profile</Link>

            <Link to={`/settings/edit-profile`} className="link pl-8 py-4">Settings</Link>

            <span className="absolute border-t border-grey  w-[100%]"></span>

            <button className="text-left p-4 hover:bg-grey w-full pl-8 py-4" onClick={signOutUser}>
                <h1 className="font-bold text-xl mg-1">Sign Out</h1>
                <p className="text-dark-grey">@{studentid}</p>
            </button>

           </div>
         </AnimatioWrapper>
    )
}

export default UserNavigationPanel